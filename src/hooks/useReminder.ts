...
const REMINDER_MESSAGES = {
  ...
  en: [
    "Logged anything today? Keep your streak alive! 🔥",
    "Don't forget to log today's spending — it only takes 10 seconds!",
    'One small note today, one good habit tomorrow. ✨',
  ],
}

const REMINDER_TITLE = { vi: 'Loop nhắc bạn 👋', en: 'Loop reminder 👋' }
...
export function useReminder(settings: UserSettings) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const messages = REMINDER_MESSAGES[lang]
      const msg = messages[Math.floor(Math.random() * messages.length)]
      const title = REMINDER_TITLE[lang]
      try {
        ...
        if (reg) {
          ...
          new Notification(title, { body: msg, icon: `${base}icons/icon-192.png` })
        }
      } catch {
  ...
  }, [settings.reminderEnabled, settings.reminderTime, loggedToday, lang])
