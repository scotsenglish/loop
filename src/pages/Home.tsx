  ...
  return (
    <div className="px-4 pt-6">
      <header className="flex items-center justify-between">
        <div>
          <p className="text-sm text-ink-400">Xin chào,</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <StatCard
          ...
          sub={
            prevExpense > 0
              ? `${expenseDelta >= 0 ? '+' : ''}${expenseDelta.toFixed(0)}% so với tháng trước`
              : undefined
          }
        />
        <StatCard
      ...
      <div className="mt-3 card-surface flex items-center justify-between rounded-2xl p-4 shadow-soft">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-ink-500">Số dư tháng này</span>
        </div>
        <span
      ...
      <section className="mt-4 pb-4">
        ...
        <div className="card-surface mt-2 rounded-2xl p-2 shadow-soft">
          {recent.length === 0 ? (
            <p className="p-4 text-center text-sm text-ink-400">Chưa có giao dịch nào</p>
          ) : (
            recent.map((tx) => (
              <TransactionRow
                key={tx.id}
